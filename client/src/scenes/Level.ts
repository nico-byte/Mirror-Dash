...
export default class Level extends Phaser.Scene {
    ...
    create() {
        ...
        this.player = new Player(this, 230, 250, this.socket, this.playername, true);
        ...
        this.socket.on(
            "playerMoved",
            (playerInfo: { id: string; x: number; y: number; direction: number; animation: string }) => {
                const otherPlayer = this.otherPlayers.get(playerInfo.id);
                if (otherPlayer) {
                    otherPlayer.setPosition(playerInfo.x, playerInfo.y);
                    otherPlayer.label.setPosition(playerInfo.x - 16, playerInfo.y - 80);
                    // otherPlayer.healthBar.setPosition(playerInfo.x - 16, playerInfo.y - 60);
